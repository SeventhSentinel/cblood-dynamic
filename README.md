# CBlood-Dynamic
A mod for [UZDoom](https://github.com/UZDoom/UZDoom) that corrects monster blood colors without replacing actors

A fork of the excellent [CBlood](https://github.com/nashmuhandes/cblood) by Nash Muhandes

License: MIT

<img width="1920" height="1080" alt="image" src="https://github.com/user-attachments/assets/f6046e9c-0611-4ea1-bcce-18f7218f572e" />

## Supported Games
- Doom, Doom II, Final Doom
- Chex Quest
- Hexen

## Supported Mapsets
- Ancient Aliens
- Back to Saturn X (all episodes)
- Baculus
- DBP61
- Disjunction
- Eviternity & Eviternity II
- Exomoon
- Fractured Worlds
- Infection
- Necromantic Thirst
- Rowdy Rudy's Revenge & Rowdy Rudy II
- Scientist 2023
- Struggle
- Sunder
- Tangerine Nightmare

### Compatibility
- CBlood-Dynamic is ZScript-based. I have only tested it in UZDoom.
- Blood graphics will look best in paletted rendering modes.

### Installation
1. [Download the latest release](https://github.com/SeventhSentinel/cblood-dynamic/releases/latest).
2. Add to your [autoload](https://zdoom.org/wiki/How_to_autoload_files).

### Do you like CBlood-Dynamic?
- Feel free to submit contributions!
- Try my status bar mod, [SevHUD](https://github.com/SeventhSentinel/sevhud)!
- CBlood-Dynamic is based on work by Nash Muhandes. Consider supporting him on [Patreon](https://www.patreon.com/NashMuhandes), or check out his game, [Disdain](https://store.steampowered.com/app/2113270/DISDAIN/).
- Check out my game, [Snap the Sentinel](https://store.steampowered.com/app/1739960/Snap_the_Sentinel/)!
